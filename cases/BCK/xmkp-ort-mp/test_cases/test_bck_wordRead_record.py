...
class TestBckWordReadRecord(BaseTestCase):
    @classmethod
    def setup_class(cls):
    ...
    def teardown_class(cls):
        LOGGER.info("TestBckWordReadRecord测试结束")

    def test_bck_wordRead_record1(self, data,setupdata):
        """
        desc: 绘本跟读钻石结算-3颗钻石
        step1:请求接口拿到返回信息
        """

        record = self.bck.bck_wordRead_record(data.recordType, data.homeworkRef,  data.tryTimes, data.score,
                                              data.audioUrl, data.originAudioUrl, data.stepTypeStr, data.text,
                                              data.stepRef, data.campRef, data.lessonRef, data.productRef,setupdata.mobile,setupdata.password)
        self.assert_equal(3, record["diamond"], "钻石数为3")
        self.assert_equal("excellent", record["comment"], "钻石评价为excellent")
        # print(record)

    def test_bck_wordRead_record2(self, data,setupdata):
        """
        desc: 绘本跟读钻石结算-2颗钻石
        step1:请求接口拿到返回信息
        """
    ...
    def test_bck_wordRead_record3(self, data,setupdata):
        """
        desc: 绘本跟读钻石结算-1颗钻石
        step1:请求接口拿到返回信息
        """
    ...
    def test_bck_wordRead_record4(self, data,setupdata):
        """
