...
def wait_for_condition(func, exit_condition, interval=1, time_out=60, **kwargs):
    ...
    while time.time() <= end_time:
        ...
        if exit_condition(resp):
            count_, total_time, spent_time, is_success = count, time_out, time.time() - start_time, True
            r = resp
            break
        count += 1
    else:
    ...
    data_t = tuple(str(i) for i in
                   (test_case_name, call_func_name, stringify_real_kwargs, total_time, spent_time, count_, is_success))
    data = ",".join(data_t)

    # 过滤广播数据请求
    if "post" != call_func_name:
        L.append(data)
