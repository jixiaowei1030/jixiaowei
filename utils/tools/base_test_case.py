...
from common.const import RespCode
from utils.decorators import checkpoint
from utils.tools.dict_format import dict_format, change_dict_key_pattern, exclude_str_root
...
class BaseTestCase(object):
    ...
    @classmethod
    @checkpoint
    def assert_iter_equal(cls, expect, actual, error_msg="", ignore_missed_expect_values=False,
    ...
    def assert_dic_type(cls, expect, actual, error_msg="", ignore_missed_expect_values=False):
        expect_ = {k: type(v) if type(v).__base__.__name__ == "object" else type(v).__base__ for k, v in expect.items()}
        actual_ = {k: type(v) if type(v).__base__.__name__ == "object" else type(v).__base__ for k, v in actual.items()}
        cls.assert_iter_equal(expect_, actual_, error_msg=error_msg,
    ...
    def assert_response(cls, resp):
        ...
        assert resp_code == RespCode.SUCCESS, cls._error_msg(RespCode.SUCCESS, resp_code, error_info)
